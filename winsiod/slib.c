...
long __stdcall
repl_c_string (char *str, long want_sigint, long want_init, long want_print)
{
  struct repl_hooks h;
  long retval;
...
set_repl_hooks (void (*puts_f) (char *),
                LISP (*read_f) (void),
                LISP (*eval_f) (LISP), void (*print_f) (LISP))
{
  repl_puts = puts_f;
  repl_read = read_f;
  repl_eval = eval_f;
...
void
set_stdout_hooks (void (*puts_f) (char *))
{
  stdout_puts = puts_f;
}
  ...
  while (1)
    {
      ...
      if (gc_kind_copying == 1)
        ...
        sprintf (tkbuffer,
                 ...
                 myruntime () - rt,
                 gc_time_taken, gc_cells_allocated, myrealtime () - ct);
      if (siod_verbose_level >= 2)
        grepl_puts (tkbuffer, h->repl_puts);
  ...
  if (errjmp_ok == 1)
    {
      ...
      for (l = catch_framep; l; l = (*l).next)
        if (EQ ((*l).tag, sym_errobj) || EQ ((*l).tag, sym_catchall))
          {
            if (!msg)
              msg = "quit";
...
{
  if TYPEP
    (x, tc_symbol) return (PNAME (x));
  else if TYPEP
    (x, tc_string) return (x->storage_as.string.data);
  else
    return (NULL);
}
...
set_gc_hooks (long type,
              LISP (*rel) (LISP),
              LISP (*mark) (LISP),
              void (*scan) (LISP), void (*free) (LISP), long *kind)
{
  ...
  switch (get_c_long (arg))
    ...
    case 4:
      return (flocons ((gc_kind_copying == 1)
                       ? (long) (heap_end - heap) : freelist_length ()));
    default:
      return (NIL);
    }
}
          ...
          switch TYPE
            ...
            case tc_subr_4:
              ...
              return (SUBR4 (tmp->storage_as.closure.code)
                      (tmp->storage_as.closure.env,
                       arg1,
                       leval (car (x), env), leval (car (cdr (x)), env)));
            case tc_subr_5:
              x = CDR (x);
              arg1 = leval (car (x), env);
              x = NULLP (x) ? NIL : CDR (x);
      ...
      switch TYPE
        ...
        case tc_subr_2:
          return (SUBR2 (fcn->storage_as.closure.code)
                  (fcn->storage_as.closure.env, car (args)));
        case tc_subr_3:
...
syntax_define (LISP args)
{
  ...
  return (syntax_define (cons (car (car (args)),
...
leval_if (LISP * pform, LISP * penv)
{
  ...
  env = *penv;
  if NNULLP
    (leval (car (args), env)) * pform = car (cdr (args));
  else
...
leval_lambda (LISP args, LISP env)
{
  LISP body;
  if NULLP
    (cdr (cdr (args))) body = car (cdr (args));
  else
...
lthrow (LISP tag, LISP value)
{
  struct catch_frame *l;
  for (l = catch_framep; l; l = (*l).next)
    if (EQ ((*l).tag, tag) || EQ ((*l).tag, sym_catchall))
      {
        (*l).retval = value;
        longjmp ((*l).cframe, 2);
      }
...
{
  LISP letb, setb, l;
  for (letb = NIL, setb = cddr (form), l = cadr (form); NNULLP (l);
       l = cdr (l))
    {
...
leval_while (LISP args, LISP env)
{
  LISP l;
  while NNULLP
    (leval (car (args), env)) for (l = cdr (args); NNULLP (l); l = cdr (l))
      leval (car (l), env);
  return (NIL);
}
...
{
  return (fopen_c
          (get_c_string (name), NULLP (how) ? "r" : get_c_string (how)));
}
...
vload (char *ofname, long cflag, long rflag)
{
  ...
  if ((start = strstr (buffer, key)))
    {
      for (end = &start[strlen (key)]; *end && isalnum (*end); ++end);
      j = end - start;
...
{
  return (vload
          (get_c_string (fname), NULLP (cflag) ? 0 : 1,
           NULLP (rflag) ? 0 : 1));
}
...
require (LISP fname)
{
  ...
  sym = intern (string_append (cons (cintern ("*"),
                                     cons (fname,
                                           cons (cintern ("-loaded*"),
                                                 NIL)))));
  if (NULLP (symbol_boundp (sym, NIL)) || NULLP (symbol_value (sym, NIL)))
    {
...
{
  return ((fseek
           (get_c_file (file, NULL), get_c_long (offset),
            get_c_long (direction))) ? NIL : sym_t);
}
...
{
  if NNULLP
    (arg) siod_verbose_level = get_c_long (car (arg));
...
lruntime (void)
{
  return (cons (flocons (myruntime ()), cons (flocons (gc_time_taken), NIL)));
}
