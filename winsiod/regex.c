...
{
  char errbuff[1024];
  regerror (get_c_long (code), get_tc_regex (ptr)->r, errbuff,
	    sizeof (errbuff));
...
regex_prin1 (LISP ptr, struct gen_printio *f)
{
  ...
  sprintf (buffer, "#<REGEX %p nsub=%d", p, (int) p->re_nsub);
  gput_st (f, buffer);
#if defined(__osf__)
  sprintf (buffer, ", len=%d flags=%X", p->re_len, p->re_cflags);
  gput_st (f, buffer);
#endif
  gput_st (f, ">");
}
...
init_regex (void)
{
  ...
  set_gc_hooks (tc_regex, NULL, NULL, NULL, regex_gc_free, &j);
