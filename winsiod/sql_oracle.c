...
allocate_cursor (void) 
{
 ...
 void
free_cursor (struct cda_def *cur) 
{
  long result;
  unsigned short rc;
  result = oclose (cur);
 ...
 LISP l_orlon (LISP username, LISP password) 
{
  ...
  if (orlon_ok)
    ...
    
    {
      flag = no_interrupt (1);
      retval =
	orlon (&lda, &hda, c_username, strlen (c_username), c_password,
...
freeloc (void **x) 
{
 ...
 void
release_statement (struct cstatement *c) 
{
  long j;
  if (c->params)
 ...
 void
prepare_statement (char *sql_str, struct cstatement *c) 
     /* assumptions:
        ...
        nselects is correct or -1                                     */ 
{
  ...
  if (c->nparams)
	...
	{
	  c->params =
	    (struct param *) malloc (sizeof (struct param) * c->nparams);
	  for (j = 1; j <= c->nparams; ++j)
	...
	
	{
	  switch (c->params[j - 1].etype)
	    ...
	    case ORACLE_ETYPE_DATE:
	      ...
	      c->params[j - 1].buf =
		(char *) malloc (c->params[j - 1].buflen);
	      break;
	    default:
	      c->params[j - 1].etype = ORACLE_ETYPE_STRING;
	      c->params[j - 1].buflen = ORACLE_MAX_STRING;
	      c->params[j - 1].buf =
		(char *) malloc (c->params[j - 1].buflen + 1);
	  }
	    if (obndrn
		   (c->cursor, j, c->params[j - 1].buf,
		      (c->params[j - 1].etype ==
			  ORACLE_ETYPE_STRING)  ? -1 : c->params[j -
								    1].
		      buflen, c->params[j - 1].etype, -1, NULL, NULL, -1,
		      -1))
  ...
  if (c->nselects)
	...
	
	{
      ...
      c->selects =
	(struct select *) malloc (sizeof (struct select) * c->nselects);
	...
	
	{
	  ...
	  if (odsc
	       (c->cursor, j, &c->selects[j - 1].dbsize, NULL, NULL,
		&c->selects[j - 1].dbtype, colnam, &colnamlen,
		&c->selects[j - 1].dsize))
	    
	    {
	      if ((!sflag) || (c->cursor->rc != ORA_VAR_NOT_IN_SELECT))
		...
		case ORACLE_ITYPE_NUMBER:
		  ...
		  c->selects[j - 1].buf =
		    (double *) malloc (c->selects[j - 1].buflen);
		...
		case ORACLE_ETYPE_DATE:
		  ...
		  c->selects[j - 1].buf =
		    (char *) malloc (c->selects[j - 1].buflen);
		  break;
		default:
		  c->selects[j - 1].etype = ORACLE_ETYPE_STRING;
		  c->selects[j - 1].buflen = ORACLE_MAX_STRING;
		  c->selects[j - 1].buf =
		    (char *) malloc (c->selects[j - 1].buflen + 1);
		  c->selects[j - 1].buf[0] = 0;
	      }
		if (odefin
		       (c->cursor, j, c->selects[j - 1].buf,
			  c->selects[j - 1].buflen,
			  c->selects[j - 1].etype, -1,
			  &c->selects[j - 1].fetchlenstat, NULL, -1, -1,
 ...
 LISP oracle_select_column_name (LISP s, LISP n) 
{
 ...
 char *
oracle_date_to_string (struct oracle_date *d) 
     /* make it look like the string returned by RDB SQL Services */ 
{
  static char buff[100];
  sprintf (buff, "%02d%02d%02d%02d%02d%02d%02d00", d->century - 100,
	    d->year - 100, d->month, d->day, d->hour - 1, d->minute - 1,
	    d->second - 1);
  return (buff);
}
...
extra_prin1 (LISP ptr, struct gen_printio *f) 
{
  ...
  switch (ptr->storage_as.string.dim)
    ...
    case extra_tc_statement:
      c = (struct cstatement *) ptr->storage_as.string.data;
      if (c->cursor)
	sprintf (buff, "#{SQL STATEMENT %p cursor %d}", c, c->cursor->cn);
