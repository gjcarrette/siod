...
static CS_RETCODE client_err_handler (CS_CONTEXT *,
				      CS_CONNECTION *, CS_CLIENTMSG *);
static CS_RETCODE server_err_handler (CS_CONTEXT *,
				      CS_CONNECTION *, CS_SERVERMSG *);

static CS_RETCODE
sybase_setup (struct sybase_state *sybase_state,
	      char *sybase_user, char *sybase_pw, char *sybase_app)
{
  ...
  status = ct_con_props (sybase_state->conn, CS_SET, CS_USERNAME,
			 sybase_user, CS_NULLTERM, (CS_INT *) NULL);
  if (status != CS_SUCCEED)
    {
  ...
  status = ct_con_props (sybase_state->conn, CS_SET, CS_PASSWORD,
			 sybase_pw, CS_NULLTERM, (CS_INT *) NULL);
  if (status != CS_SUCCEED)
    {
  ...
  status = ct_con_props (sybase_state->conn, CS_SET, CS_APPNAME,
			 sybase_app, CS_NULLTERM, (CS_INT *) NULL);
  if (status != CS_SUCCEED)
    {
  ...
  if (gethostname (hostname, sizeof (hostname)) == 0)
    {
      status = ct_con_props (sybase_state->conn, CS_SET, CS_HOSTNAME,
			     hostname, CS_NULLTERM, (CS_INT *) NULL);
      if (status != CS_SUCCEED)
	{
  ...
  if (status != CS_SUCCEED)
    {
      complain ("ct_connent(%s,%s,%s) %d",
		sybase_user, sybase_pw, sybase_app, status);
      sybase_teardown (sybase_state);
      return (status);
    }
...
static CS_RETCODE
sybase_teardown (struct sybase_state *sybase_state)
{
  CS_RETCODE status = CS_SUCCEED;
  if (sybase_state->cmd)
    {
...
static char *
getarg (LISP l, char *name, char *dflt)
{
  LISP cell, key;
  key = rintern (name);
  for (cell = l; NNULLP (cell); cell = cdr (cdr (cell)))
    if EQ
      (car (cell), key) return (get_c_string (cadr (cell)));
  return (dflt);
}

static LISP
statcons (CS_RETCODE n)
{
  char *str;
...
struct sybase_state *
get_sybase_state (LISP ptr, long openp)
{
...
sybase_open (LISP args)
{
  ...
  if NULLP
    (leval (sym_sybase, NIL)) setvar (sym_sybase, value, NIL);
  return (value);
}

LISP
sybase_close (LISP value)
{
  int iflag;
  CS_RETCODE status;
  LISP obj;
  setvar (sym_sybase_messages, NIL, NIL);
  if NULLP
    (value) obj = leval (sym_sybase, NIL);
  else
    obj = value;
  iflag = no_interrupt (1);
  ...
  if EQ
    (leval (sym_sybase, NIL), obj) setvar (sym_sybase, NIL, NIL);
  no_interrupt (iflag);
  return (statcons (status));
}

LISP
sybase_status (LISP value)
{
  int iflag;
  CS_RETCODE status;
  CS_INT mask, retlen;
...
fetch_data (CS_COMMAND * cmd, LISP * header, LISP * items)
{
  CS_RETCODE retcode;
  ...
  for (i = 0; i < num_cols; ++i)
      ...
      if (data->value)
	{
      ...
      if (siod_verbose_check (5))
	printf (";; %d %.*s %s[%d] %d\n", (int) i, (int) fmt->namelen,
		fmt->name,
		(sybase_typestr (fmt->datatype)) ? sybase_typestr (fmt->
								   datatype) :
		"?", (int) fmt->maxlength, (int) fmt->usertype);
      switch (fmt->datatype)
	{
	...
	default:
      ...
      retcode = ct_bind (cmd, (i + 1), fmt,
			 data->value, &data->valuelen, &data->indicator);
      if (retcode != CS_SUCCEED)
	{
  ...
  while (((retcode = ct_fetch (cmd, CS_UNUSED, CS_UNUSED, CS_UNUSED,
      ...
      for (i = 0; i < num_cols; i++)
	{
	  ...
	  if (data->indicator >= 0)
		...
		memcpy (row->storage_as.lisp_array.data[i]->storage_as.string.
			data, data->value, fmt->maxlength);
		break;
	      default:
		errswitch ();
	      }
...
sybase_execute (LISP arglist)
{
  ...
  obj = poparg (&l, NIL);
  if TYPEP
    (obj, tc_sybase_state) str = poparg (&l, NIL);
  else
    {
      str = obj;
  ...
  if (!param_fmt)
    {
      param_fmt =
	(CS_DATAFMT *) malloc (sizeof (CS_DATAFMT) * MAX_PARAM_ARGS);
  ...
  for (j = 0, l = args; NNULLP (l); l = cddr (l))
	...
	err ("too many arguments for sybase-execute to handle", args);
      ...
      m = strlen (argname);
      if (m >= (CS_MAX_NAME - 1))
	err ("argument name too long", caar (l));
	...
	default:
      ...
      status = ct_param (sybase_state->cmd,
			 &param_fmt[j], data, datalen, CS_UNUSED);
      if (status != CS_SUCCEED)
	{
...
void
push_sybase_messages (LISP value)
{
  setvar (sym_sybase_messages,
	  cons (value, leval (sym_sybase_messages, NIL)), NIL);
}
...
server_err_handler (CS_CONTEXT * cp, CS_CONNECTION * chp, CS_SERVERMSG * msgp)
{
  ...
  if (docomplain)
    complain ("Server: msgno(%d) severity(%d) state(%d)\n%s",
	      msgp->msgnumber, msgp->severity, msgp->state, msgp->text);
  (push_sybase_messages
   (listn (5,
	   ...
	   cons (cintern ("text"),
		 strcons (strlen (msgp->text), msgp->text)))));
  return (CS_SUCCEED);
}

static CS_RETCODE
client_err_handler (CS_CONTEXT * cp,
		    CS_CONNECTION * chp, CS_CLIENTMSG * emsgp)
{
  LISP note;
...
cs_err_handler (CS_CONTEXT * cp, CS_CLIENTMSG * msg)
{
  ...
  if (msg->osstringlen > 0)
    complain ("CS-Library OS error %ld - %s.", msg->osnumber, msg->osstring);
  note = listn (6,
		cintern ("cs-library"),
		cons (cintern ("layer"), flocons (CS_LAYER (msg->msgnumber))),
		cons (cintern ("origin"),
		      flocons (CS_ORIGIN (msg->msgnumber))),
		cons (cintern ("severity"),
		      flocons (CS_SEVERITY (msg->msgnumber))),
		cons (cintern ("number"),
		      flocons (CS_NUMBER (msg->msgnumber))),
		cons (cintern ("msgstring"),
...
void
sybase_prin1 (LISP ptr, struct gen_printio *f)
{
  char buff[256];
...
void
init_sql_sybase (void)
{
  long j;
  tc_sybase_state = allocate_user_tc ();
  set_gc_hooks (tc_sybase_state, NULL, NULL, NULL, sybase_gc_free, &j);
