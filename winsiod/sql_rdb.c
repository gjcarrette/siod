...
LISP
list3 (LISP a, LISP b, LISP c)
{
  return (cons (a, cons (b, cons (c, NIL))));
}

void
ps_i (char *fmt, long n)
{
  char buff[512];
  sprintf (buff, fmt, n);
  put_st (buff);
...
static LISP
extcons (long length, long typec)
{
  long flag;
  LISP s;
  flag = no_interrupt (1);
...
sqlrtl_associate (LISP l)
{
  ...
  status = sqlsrv_associate (nodename,
			     ...
			     a->read_buffer_size,
			     a->write_buffer_size, 0, &associate_str, &a->id);
  if (status == SQL_SUCCESS)
    {
...
struct association *
get_association (LISP assoc, long oflag)
{
  struct association *a;
...
LISP
sqlrtl_associations (void)
{
  return (associations);
}

LISP
sqlrtl_release (LISP assoc)
{
  struct association *a;
  long iflag, status;
  char errmsg[512];
...
LISP
sqlrtl_status_error (long status)
{
  char errmsg[512];
...
LISP
sqlrtl_sqlca_num_batch_rows (LISP assoc)
{
  long status, iflag;
  struct association *a;
  a = get_association (assoc, 1);
...
LISP
sqlrtl_execute_immediate (LISP assoc, LISP stmt)
{
  long status, iflag;
  char *st;
  struct association *a;
...
LISP
sqlrtl_describe_association (LISP assoc)
{
  struct association *a;
...
sqlrtl_sqlda_alist (SQLDA_ID x)
{
  ...
  for (i = 0; i < n; ++i)
	...
	case SQLSRV_ASCII_STRING:
	  vtype = sym_ascii_string;
	  break;
	case SQLSRV_GENERALIZED_NUMBER:
	  vtype = sym_generalized_number;
	  break;
	case SQLSRV_GENERALIZED_DATE:
	  vtype = sym_generalized_date;
	  break;
	case SQLSRV_VARCHAR:
	  vtype = sym_varchar;
	  break;
...
LISP
sqlrtl_prepare (LISP assoc, LISP sql)
{
...
LISP
sqlrtl_statement_params (LISP x)
{
  return ((get_statement (x))->param_alist);
}

LISP
sqlrtl_statement_selects (LISP x)
{
  return ((get_statement (x))->select_alist);
}

LISP
sqlrtl_statement_association (LISP x)
{
  return ((get_statement (x))->association);
}

void
describe_sqlda (SQLDA_ID x)
{
  long i, n;
  char name[100], *vtype;
  short namelen, type, scale;
  ...
  for (i = 0; i < n; ++i)
	...
	case SQLSRV_ASCII_STRING:
	  ...
	  break;
	case SQLSRV_GENERALIZED_NUMBER:
	  vtype = "GENERALIZED_NUMBER";
	  break;
	case SQLSRV_GENERALIZED_DATE:
	  vtype = "GENERALIZED_DATE";
	  break;
	case SQLSRV_VARCHAR:
	  vtype = "VARCHAR";
	  break;
...
LISP
sqlrtl_describe_statement (LISP x)
{
  struct statement *c;
  c = get_statement (x);
...
LISP
sqlrtl_release_statement (LISP x)
{
  struct statement *c;
  struct association *a;
  LISP assoc;
...
LISP
sqlrtl_declare_cursor (LISP stmt, LISP type, LISP mode)
{
  ...
  cursor = c->cursor;
  if NULLP
    (cursor) return (err ("statement has no cursor", stmt));
  if (NULLP (type) && NULLP (mode))
    return (NIL);
  else
    {
      if EQ
	(type, sym_table) itype = SQLSRV_TABLE_CURSOR;
      else if EQ
	(type, sym_list) itype = SQLSRV_LIST_CURSOR;
      else
	err ("invalid cursor type", type);
      if EQ
	(mode, sym_update) imode = SQLSRV_MODE_UPDATE;
      else if EQ
	(mode, sym_read_only) imode = SQLSRV_MODE_READ_ONLY;
      else if EQ
	(mode, sym_insert_only) imode = SQLSRV_MODE_INSERT_ONLY;
      else
	err ("invalid cursor mode", mode);
    }
  iflag = no_interrupt (1);
  status =
    sqlsrv_declare_cursor (a->id, get_c_string (cursor), c->id, itype, imode);
  if (status == SQL_SUCCESS)
    {
      no_interrupt (iflag);
      return (nullp (NIL));
...
LISP
sqlrtl_execute (LISP stmt, LISP batchp)
{
  struct statement *c;
  struct association *a;
  ...
  if NULLP
    ...
    (batchp, sym_begin) eflag = 1;
  else if EQ
    (batchp, sym_end) eflag = 2;
  else if EQ
    (batchp, sym_abort) eflag = 3;
  else
...
sqlrtl_open_cursor (LISP stmt)
{
  ...
  cursor = c->cursor;
  if NULLP
    (cursor) err ("statement has no cursor", stmt);
  iflag = no_interrupt (1);
  status =
    sqlsrv_open_cursor (a->id, get_c_string (cursor), c->id, c->params);
  if (status == SQL_SUCCESS)
    {
      no_interrupt (iflag);
      return (nullp (NIL));
...
LISP
sqlrtl_fetch_many (LISP stmt, LISP count)
{
  struct statement *c;
  struct association *a;
  long status, iflag, k;
...
LISP
sqlrtl_get_datum (SQLDA_ID x, long k)
{
...
long
get_datum_index (LISP n, LISP alist)
{
  LISP tmp;
  if NNULLP
    (numberp (n)) tmp = n;
  else if NNULLP
    (tmp = assq (n, alist)) tmp = cadr (tmp);
  else
...
LISP
sqlrtl_get_param (LISP x, LISP n)
{
  struct statement *c;
  c = get_statement (x);
...
LISP
sqlrtl_get_column (LISP x, LISP n)
{
  struct statement *c;
  c = get_statement (x);
  return (sqlrtl_get_datum
	  (c->selects, get_datum_index (n, c->select_alist)));
}

void
sqlrtl_put_datum (SQLDA_ID x, long k, LISP value)
{
  short typ, scl, nullp;
...
LISP
sqlrtl_set_param (LISP x, LISP n, LISP value)
{
  struct statement *c;
  c = get_statement (x);
  sqlrtl_put_datum (c->params, get_datum_index (n, c->param_alist), value);
  return (NIL);
}

LISP
sqlrtl_release_generic (LISP x)
{
  if ((TYPE (x) != tc_extra) ||
...
LISP
sqlrtl_error_buffer (LISP assoc, LISP resetp)
{
  struct association *a;
  long iflag, len;
  char *end;
...
LISP
sqlrtl_association_statements (LISP assoc)
{
  struct association *a;
  a = get_association (assoc, 0);
  return (a->statements);
...
void
extra_prin1 (LISP ptr, struct gen_printio *f)
{
  struct association *a;
  struct statement *s;
  char buff[512];
...
init_sql_rdb (void)
{
  ...
  set_gc_hooks (tc_extra,
		NULL, extra_gc_mark, extra_gc_scan, extra_gc_free, &j);
  ...
  init_subr_1 ("rdb-sql-association-statements",
	       sqlrtl_association_statements);
