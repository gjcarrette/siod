...
LISP
lgetservice (LISP lport, LISP lproto)
{
  long iflag, j;
  LISP result = NIL;
  struct servent *p;
...
LISP
s_open (LISP lhost, LISP lport, LISP aflag)
        ...
        followed by an accept. */
{
  ...
  if NULLP
      ...
      if ((status = bind (sd, (struct sockaddr *) &local, sizeof (local))))
	{
      ...
      if ((status =
	   connect (sd, (struct sockaddr *) &remote, sizeof (remote))))
	{
	  save_errno = errno;
	  close (sd);
  ...
  else
    {
      iv = 1;
      if (setsockopt
	  (sd, SOL_SOCKET, SO_REUSEADDR, (char *) &iv, sizeof (iv)))
	{
	  save_errno = errno;
	  close (sd);
...
LISP
decode_hostent (struct hostent * p)
{
  LISP name;
...
static int
select_read_tmo (int sd, double tmo)
{
  int retval;
...
struct sock_stream *
get_ss (LISP s, long openchk)
{
  if NTYPEP
    (s, tc_sock_stream) err ("not a socket stream", s);
...
int
ss_filbuf (struct sock_stream *ss)
{
  int status;
  ss->icnt = 0;
...
void
ss_force (struct sock_stream *ss)
{
  int status, size, j;
...
int
ss_flsbuf (int c, struct sock_stream *ss)
{
  ss_force (ss);
  --(ss)->ocnt;
  *(ss)->optr++ = c;
...
void
ss_prin1 (LISP s, struct gen_printio *f)
{
  char buff[512];
  unsigned char *p;
  struct sock_stream *ss;
...
#ifndef WIN32
LISP l_getname (int (*fcn) (int fn, struct sockaddr *, socklen_t *),
		char *msg, LISP s)
{
  struct sock_stream *ss = get_ss (s, 1);
  struct sockaddr_in a;
  ...
  p = ((unsigned char *) &a.sin_addr.s_addr);
  sprintf (buff, "%d.%d.%d.%d:%d", (int) p[0], (int) p[1], (int) p[2],
	   (int) p[3], (int) ntohs (a.sin_port));
  return (strcons (-1, buff));
}

LISP
l_getsockname (LISP s)
{
  return (l_getname (getsockname, "getsockname", s));
}

LISP
l_getpeername (LISP s)
{
...
int
ss_getc_fcn (struct sock_stream *ss)
{
  int c, iflag;
  iflag = no_interrupt (1);
  c = SS_GETC (ss);
...
void
ss_ungetc_fcn (int c, struct sock_stream *ss)
{
  int iflag;
  if (c == EOF)
    return;
...
lwsa_data (void)
{
  return (symalist ("version", cons (flocons (wsa_data.wHighVersion),
		    ...
		    "max-udp-dg", flocons (wsa_data.iMaxUdpDg), NULL));
}
#endif
...
init_ss (void)
{
  ...
  set_gc_hooks (tc_sock_stream, NULL, NULL, NULL, ss_gc_free, &j);
