...
init_storage_a1 (long type)
{
  ...
  set_gc_hooks (type,
		array_gc_relocate,
		array_gc_mark, array_gc_scan, array_gc_free, &j);
...
array_gc_scan (LISP ptr)
{
  long j;
  if TYPEP
    (ptr, tc_lisp_array) for (j = 0; j < ptr->storage_as.lisp_array.dim; ++j)
...
print_to_string (LISP exp, LISP str, LISP nostart)
{
  ...
  s.cb_argument = str;
  if NULLP
    (nostart) str->storage_as.string.data[0] = 0;
  lprin1g (exp, &s);
  return (str);
}
...
arcons (long typecode, long n, long initp)
{
  ...
  switch (typecode)
    ...
    case tc_long_array:
      ...
      a->storage_as.long_array.data =
	(long *) must_malloc (n * sizeof (long));
      if (initp)
	for (j = 0; j < n; ++j)
    ...
    case tc_lisp_array:
      ...
      a->storage_as.lisp_array.data =
	(LISP *) must_malloc (n * sizeof (LISP));
      for (j = 0; j < n; ++j)
...
array_equal (LISP a, LISP b)
{
  ...
  switch (TYPE (a))
	...
	return (NIL);
      if (memcmp (a->storage_as.string.data, b->storage_as.string.data, len)
	  == 0)
	return (sym_t);
      else
	return (NIL);
	...
	return (NIL);
      if (memcmp (a->storage_as.long_array.data,
		  b->storage_as.long_array.data, len * sizeof (long)) == 0)
	return (sym_t);
      else
	return (NIL);
    case tc_double_array:
	...
	if NULLP
	  (equal (a->storage_as.lisp_array.data[j],
		  b->storage_as.lisp_array.data[j])) return (NIL);
      return (sym_t);
    default:
      return (errswitch ());
    }
...
array_sxhash (LISP a, long n)
{
  ...
  switch (TYPE (a))
    ...
    case tc_byte_array:
      len = a->storage_as.string.dim;
      for (j = 0, hash = 0, char_data =
	   (unsigned char *) a->storage_as.string.data; j < len;
	   ++j, ++char_data)
	hash = HASH_COMBINE (hash, *char_data, n);
      return (hash);
    case tc_long_array:
      len = a->storage_as.long_array.dim;
      for (j = 0, hash = 0, long_data =
	   (unsigned long *) a->storage_as.long_array.data; j < len;
	   ++j, ++long_data)
...
href (LISP table, LISP key)
{
  return (cdr (assoc (key,
		      table->storage_as.lisp_array.
		      data[href_index (table, key)])));
}
...
hset (LISP table, LISP key, LISP value)
{
  ...
  if NNULLP
    (cell = assoc (key, l)) return (setcdr (cell, value));
  cell = cons (key, value);
...
void
must_fread (void *buffer, size_t elem_size, size_t elem_count, FILE * f)
{
...
fast_print_table (LISP obj, LISP table)
{
  ...
  if NULLP
    (index = car (cdr (cdr (table)))) return (1);
  hset (ht, obj, index);
  FLONM (bashnum) = 1.0;
  ...
  switch (TYPE (obj))
    ...
    case tc_flonum:
      ...
      fwrite (&obj->storage_as.flonum.data,
	      sizeof (obj->storage_as.flonum.data), 1, f);
      return (NIL);
    case tc_symbol:
  ...
  switch (c)
    ...
    case tc_flonum:
      ...
      must_fread (&tmp->storage_as.flonum.data,
		  sizeof (tmp->storage_as.flonum.data), 1, f);
      return (tmp);
    case tc_symbol:
      len = get_long (f);
      if (len >= TKBUFFERN)
...
array_fast_read (int code, LISP table)
{
  ...
  switch (code)
    ...
    case tc_byte_array:
      ...
      ptr->storage_as.string.data = (char *) must_malloc (len);
      must_fread (ptr->storage_as.string.data, sizeof (char), len, f);
      no_interrupt (iflag);
      return (ptr);
    case tc_double_array:
...
number2string (LISP x, LISP b, LISP w, LISP p)
{
  ...
  if (NULLP (b) || EQ (sym_e, b) || EQ (sym_f, b))
	...
	sprintf (buffer, NULLP (b) ? "%g" : EQ (sym_e, b) ? "%e" : "%f", y);
    }
...
lstrunbreakup (LISP elems, LISP lmarker)
{
  ...
  for (l = elems, result = NIL; NNULLP (l); l = cdr (l))
    if EQ
      (l, elems) result = cons (car (l), result);
    else
     ...
     /* this is a stupid recursive qsort */
{
  ...
  return (nconc (lqsort (less, f, g), cons (mark, lqsort (notless, f, g))));
}
...
mapcar2 (LISP fcn, LISP in1, LISP in2)
{
  ...
  for (l1 = cdr (in1), l2 = cdr (in2); CONSP (l1) && CONSP (l2);
       l1 = CDR (l1), l2 = CDR (l2))
...
lsubset (LISP fcn, LISP l)
{
  ...
  for (v = l; CONSP (v); v = CDR (v))
    if NNULLP
      (funcall1 (fcn, CAR (v))) result = cons (CAR (v), result);
  return (nreverse (result));
}
...
append (LISP l)
{
  ...
  if NULLP
    ...
    (cddr (l)) return (append2 (car (l), cadr (l)));
  else
...
LISP
fast_save (LISP fname, LISP forms, LISP nohash, LISP comment, LISP fmode)
{
  ...
  stream = listn (3,
		  fopen_c (cname,
			   NNULLP (fmode) ? get_c_string (fmode) : "wb"),
		  NNULLP (nohash) ? NIL : cons_array (flocons (100), NIL),
		  flocons (0));
  f = get_c_file (car (stream), NULL);
  if NNULLP
    (comment) fput_st (f, get_c_string (comment));
...
swrite (LISP stream, LISP table, LISP data)
{
  ...
  switch (TYPE (data))
	...
	err ("no object repeat count", data);
      m = get_c_long (swrite2 (data->storage_as.lisp_array.data[0], table));
      for (k = 0; k < m; ++k)
	for (j = 1; j < n; ++j)
...
LISP
substring_equalcase (LISP str1, LISP str2, LISP start, LISP end)
{
  char *cstr1, *cstr2;
  long len1, n, s, e;
  cstr1 = get_c_string_dim (str1, &len1);
  ...
  if ((s < 0) || (s > e) || (e < 0) || (e > n) || ((e - s) != len1))
    return (NIL);
  return ((strncasecmp (cstr1, &cstr2[s], e - s) ==
	   0) ? a_true_value () : NIL);
}
