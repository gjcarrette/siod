...
LISP
ldbm_open (LISP lfname, LISP lflags, LISP lmode)
{
  mode_t mode;
  int flags;
  char *fname;
...
LISP
ldbm_fetch (LISP ldbm, LISP lkey)
{
  long iflag;
  DBM *db;
  char *key;
...
LISP
ldbm_delete (LISP ldbm, LISP lkey)
{
  long iflag, status;
  DBM *db;
  char *key;
...
LISP
ldbm_store (LISP ldb, LISP lkey, LISP ldata, LISP lflags)
{
  long iflag, status;
  DBM *db;
  char *key, *data;
...
void
dbm_prin1 (LISP ptr, struct gen_printio *f)
{
  char buff[256];
  DBM *p;
  p = get_DBM (ptr, 0);
...
init_ndbm (void)
{
  ...
  set_gc_hooks (tc_dbm, NULL, NULL, NULL, dbm_gc_free, &j);
