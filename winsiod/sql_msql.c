  ...
  if (thisrow = msqlFetchRow (res))
      ...
      for (i = 0; i < count; i++)
	{
	  ...
	  if (thisrow[i])
		...
		{
		case INT_TYPE:
		  liste->storage_as.cons.car =
		    strcons (strlen (thisrow[i]), thisrow[i]);
		  s1 = cons (NIL, NIL);
...
LISP
msql_data_seek (LISP s, LISP cursor)
{
  long flag;
  int count, counter;
  int i;
...
msql_list_tables (void)
{
  ...
  while (thisrow = msqlFetchRow (res))
    {
      liste->storage_as.cons.car =
	strcons (strlen (thisrow[0]) + 1, thisrow[0]);
      s1 = cons (NIL, NIL);
      liste->storage_as.cons.cdr = s1;
      liste = liste->storage_as.cons.cdr;
    }
