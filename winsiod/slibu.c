...
LISP
ldecode_pwent (struct passwd * p)
{
  return (symalist ("name", strcons (strlen (p->pw_name), p->pw_name),
		    ...
		    "comment", strcons (strlen (p->pw_comment),
					p->pw_comment),
#endif
...
static char *
strfield (char *name, LISP alist)
{
  LISP value, key = rintern (name);
  if NULLP
    (value = assq (key, alist)) return ("");
  return (get_c_string (cdr (value)));
}

static long
longfield (char *name, LISP alist)
{
  LISP value, key = rintern (name);
  if NULLP
    (value = assq (key, alist)) return (0);
  return (get_c_long (cdr (value)));
}

void
lencode_pwent (LISP alist, struct passwd *p)
{
...
LISP
lputpwent (LISP alist, LISP file)
{
  int iflag = no_interrupt (1);
  int status, xerrno;
  struct passwd p;
...
LISP
laccess_problem (LISP lfname, LISP lacc)
{
...
LISP
lexec (LISP path, LISP args, LISP env)
{
  int iflag;
...
LISP
lkill (LISP pid, LISP sig)
{
  long iflag;
  iflag = no_interrupt (1);
  if (kill (get_c_long (pid), NULLP (sig) ? SIGKILL : get_c_long (sig)))
...
lgetgrgid (LISP n)
{
  ...
  if ((gr = getgrgid (gid)))
	...
	result =
	  cons (strcons (strlen (gr->gr_mem[j]), gr->gr_mem[j]), result);
...
void
md5_update_from_file (MD5_CTX * ctx, FILE * f, unsigned char *buff, long dim)
{
  size_t len;
...
LISP
md5_update (LISP ctx, LISP str, LISP len)
{
  char *buffer;
  long dim, n;
  ...
  if TYPEP
  ...
  else
    {
      n = get_c_long (len);
      if ((n < 0) || (n > dim))
	err ("invalid length for string", len);
    }
...
LISP
cpu_usage_limits (LISP soft, LISP hard)
{
  struct rlimit x;
...
LISP
current_resource_usage (LISP kind)
{
  struct rusage u;
  int code;
  if (NULLP (kind) || EQ (cintern ("SELF"), kind))
    ...
    (cintern ("CHILDREN"), kind) code = RUSAGE_CHILDREN;
  else
  ...
  return (symalist ("utime", flocons (((double) u.ru_utime.tv_sec) +
		    ...
		    "nivcsw", flocons (u.ru_nivcsw), NULL));
}
...
DIR *
get_opendir (LISP v, long oflag)
{
  if NTYPEP
    (v, tc_opendir) err ("not an opendir", v);
  if NULLP
    (CAR (v))
    {
      if (oflag)
...
void
opendir_prin1 (LISP ptr, struct gen_printio *f)
{
  char buffer[256];
...
LISP
llast_win32_errmsg (DWORD status)
{
  DWORD len, msgcode;
  char buffer[256];
  msgcode = (status == 0) ? GetLastError () : status;
  len = FormatMessage (FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_MAX_WIDTH_MASK, 0, msgcode, 0,	/* what language? */
...
l_opendir (LISP name)
{
  ...
  CAR (value) = (LISP) d;
  if ((d->h =
       FindFirstFile (get_c_string (name), &d->s)) == INVALID_HANDLE_VALUE)
...
#if defined(unix)
LISP
decode_st_moden (mode_t mode)
{
  LISP ret = NIL;
  if (mode & S_ISUID)
    ret = cons (cintern ("SUID"), ret);
  if (mode & S_ISGID)
    ret = cons (cintern ("SGID"), ret);
  if (mode & S_IRUSR)
    ret = cons (cintern ("RUSR"), ret);
  if (mode & S_IWUSR)
    ret = cons (cintern ("WUSR"), ret);
  if (mode & S_IXUSR)
    ret = cons (cintern ("XUSR"), ret);
  if (mode & S_IRGRP)
    ret = cons (cintern ("RGRP"), ret);
  if (mode & S_IWGRP)
    ret = cons (cintern ("WGRP"), ret);
  if (mode & S_IXGRP)
    ret = cons (cintern ("XGRP"), ret);
  if (mode & S_IROTH)
    ret = cons (cintern ("ROTH"), ret);
  if (mode & S_IWOTH)
    ret = cons (cintern ("WOTH"), ret);
  if (mode & S_IXOTH)
    ret = cons (cintern ("XOTH"), ret);
  if (S_ISFIFO (mode))
    ret = cons (cintern ("FIFO"), ret);
  if (S_ISDIR (mode))
    ret = cons (cintern ("DIR"), ret);
  if (S_ISCHR (mode))
    ret = cons (cintern ("CHR"), ret);
  if (S_ISBLK (mode))
    ret = cons (cintern ("BLK"), ret);
  if (S_ISREG (mode))
    ret = cons (cintern ("REG"), ret);
  if (S_ISLNK (mode))
    ret = cons (cintern ("LNK"), ret);
  if (S_ISSOCK (mode))
...
LISP
decode_st_mode (LISP value)
{
  return (decode_st_moden (get_c_long (value)));
}

LISP
decode_stat (struct stat * s)
{
...
LISP
g_stat (LISP fname, int (*fcn) (const char *, struct stat *))
{
  struct stat st;
  int iflag, ret;
  iflag = no_interrupt (1);
...
LISP
l_fnmatch (LISP pat, LISP str, LISP flgs)
{
  if (fnmatch (get_c_string (pat), get_c_string (str), 0))
    return (NIL);
  else
    return (a_true_value ());
}
...
LISP
lutime (LISP fname, LISP mod, LISP ac)
{
  struct utimbuf x;
...
{
  if TYPEP
    (ptr, tc_c_file) return (fileno (get_c_file (ptr, NULL)));
  else
    return (get_c_long (ptr));
}

LISP
gsetlk (int op, LISP lfd, LISP ltype, LISP whence, LISP start, LISP len)
{
  struct flock f;
  int fd = get_fd (lfd);
...
LISP
lF_SETLK (LISP fd, LISP ltype, LISP whence, LISP start, LISP len)
{
  return (gsetlk (F_SETLK, fd, ltype, whence, start, len));
}

LISP
lF_SETLKW (LISP fd, LISP ltype, LISP whence, LISP start, LISP len)
{
  return (gsetlk (F_SETLKW, fd, ltype, whence, start, len));
}

LISP
lF_GETLK (LISP fd, LISP ltype, LISP whence, LISP start, LISP len)
{
...
set_dsc_cst (struct dsc$descriptor *d, char *s)
{
...
LISP
lset_logical (LISP name, LISP value, LISP table, LISP attributes)
{
  ...
  status = lib$set_logical (set_dsc_cst (&dname, get_c_string (name)),
			    ...
			    NULLP (table) ? 0 : set_dsc_cst (&dtable,
							     get_c_string
							     (table)),
			    assemble_options (attributes, "NO_ALIAS",
					      LNM$M_NO_ALIAS, "CONFINE",
					      LNM$M_CONFINE, "CRELOG",
					      LNM$M_CRELOG, "TABLE",
					      LNM$M_TABLE, "CONCEALED",
					      LNM$M_CONCEALED, "TERMINAL",
					      LNM$M_TERMINAL, "EXISTS",
					      LNM$M_EXISTS, "SHAREABLE",
					      LNM$M_SHAREABLE, "CREATE_IF",
					      LNM$M_CREATE_IF, "CASE_BLIND",
					      LNM$M_CASE_BLIND, NULL), 0);
...
LISP
unix_ctime (LISP value)
{
  time_t b;
  char *buff, *p;
  if NNULLP
    (value) b = get_c_long (value);
  else
    time (&b);
  if ((buff = ctime (&b)))
    {
     ...
     /* returns the internet standard RFC 1123 format */
{
  ...
  (sprintf
   (buff, "%s, %02d %s %04d %02d:%02d:%02d GMT",
    ...
    t->tm_mday,
    &"Jan\0Feb\0Mar\0Apr\0May\0Jun\0Jul\0Aug\0Sep\0Oct\0Nov\0Dec"[t->tm_mon *
								  4],
...
#ifdef hpux
long
usleep (unsigned int winks)	/* added, dcd */
{
...
lgets (LISP file, LISP buffn)
{
  ...
  if NULLP
    (buffn) n = sizeof (buffer);
  else if ((n = get_c_long (buffn)) < 0)
    err ("size must be >= 0", buffn);
  else if (n > sizeof (buffer))
    err ("not handling buffer of size",
	 listn (2, buffn, flocons (sizeof (buffer))));
  iflag = no_interrupt (1);
...
LISP
l_chown (LISP path, LISP uid, LISP gid)
{
  long iflag;
  iflag = no_interrupt (1);
...
LISP
l_lchown (LISP path, LISP uid, LISP gid)
{
  long iflag;
  iflag = no_interrupt (1);
...
LISP
so_init_name (LISP fname, LISP iname)
{
  LISP init_name;
  if NNULLP
    (iname) init_name = iname;
  else
    {
      init_name = car (last (lstrbreakup (fname, cintern ("/"))));
#if !defined(VMS)
      init_name =
	lstrunbreakup (butlast (lstrbreakup (init_name, cintern ("."))),
		       cintern ("."));
#endif
     ...
     /* note: error cases can leak memory in this procedure. */
{
  ...
  status = lib$find_image_symbol (set_dsc_cst (&filename,
				  ...
				  &fcn, NULLP (dsym) ? 0 : &defaultd);
  if (status != SS$_NORMAL)
    err_vms (status);
#endif
#ifdef WIN32
  fname = string_append (listn (2, strcons (-1, siod_lib), fname));
...
LISP
ccall_catch_1 (LISP (*fcn) (void *), void *arg)
{
  LISP val;
  val = (*fcn) (arg);
...
LISP
ccall_catch (LISP tag, LISP (*fcn) (void *), void *arg)
{
...
void
encode_tm (LISP alist, struct tm *t)
{
  LISP val;
...
LISP
lstrptime (LISP str, LISP fmt, LISP in)
{
  struct tm tm;
  encode_tm (in, &tm);
...
LISP
err_large_index (LISP ind)
{
  return (err ("index too large", ind));
}

LISP
datref (LISP dat, LISP ctype, LISP ind)
{
  char *data;
  long size, i;
...
LISP
sdatref (LISP spec, LISP dat)
{
  return (datref (dat, car (spec), cdr (spec)));
}

LISP
mkdatref (LISP ctype, LISP ind)
{
  return (closure (cons (ctype, ind), leval (cintern ("sdatref"), NIL)));
}

LISP
datlength (LISP dat, LISP ctype)
{
  long size;
  ...
  if (CONSP (result) && TYPEP (car (result), tc_string))
    {
      ...
      put_st
	("<HTML><HEAD><TITLE>Server Error (Application)</TITLE></HEAD>\n");
...
htqs_arg (char *value)
{
  ...
  if ((strcmp (value, "(repl)") == 0) || (strcmp (value, "repl") == 0))
...
int __stdcall
siod_main (int argc, char **argv, char **env)
{
...
/* A megabyte seems like a huge bootstrap exe,
   but we have seen that in debug mode the size of wcsiod.exe,
   a mere 50 lines of source code, as grown to nearly 500KB
*/

#define BOOTSTRAP_EXE_FILE_SIZE_LIMIT 1000000

long
position_script (FILE * f, char *buff, size_t bufflen)
   ...
   of the sequence showing up in a file of N random bytes. */
{
  ...
  size_t j;
  buff[0] = 0;
  for (offset = 0; offset < BOOTSTRAP_EXE_FILE_SIZE_LIMIT; ++offset)
    {
      c = getc (f);
      switch (c)
	{
...
void __stdcall
siod_shuffle_args (int *pargc, char ***pargv)
...
LISP
lposition_script (LISP lfile)
{
  FILE *f;
  long iflag, pos;
  char flbuff[100];
  if (SYMBOLP (lfile))
    return (flocons (BOOTSTRAP_EXE_FILE_SIZE_LIMIT));
...
init_slibu (void)
{
  ...
  set_gc_hooks (tc_opendir, NULL, NULL, NULL, opendir_gc_free, &j);
...
#ifdef unix
  if ((!(tmp1 = getenv (ld_library_path_env))) || (!strstr (tmp1, siod_lib)))
    {
