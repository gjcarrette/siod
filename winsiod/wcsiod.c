...
static char siod_lib_arg[128] = "-l";
 static char *siod_argv[7] =
  { "siod", "-h1000000:50", "-g0", "-o1000", "-s200000", "-n2048",
siod_lib_arg
};
...
main (int argc, char **argv, char **env) 
{
  ...
  HKEY siodkey;
  for (j = 1; j < argc; ++j)
    if ((strncmp (argv[j], "-v", 2) == 0) && (atol (&argv[j][2]) > 4))
      debugflag = 1;
  if (debugflag)
    printf ("Registry key %s\n", registry_key);
  status =
    RegOpenKeyEx (HKEY_LOCAL_MACHINE, registry_key, 0, KEY_QUERY_VALUE,
		  &siodkey);
  if (status == ERROR_SUCCESS)
	...
	printf ("opened, status %d\n", status);
      ...
      status =
	RegQueryValueEx (siodkey, "SIOD_LIB", NULL, &vtype,
			 &siod_lib_arg[2], &vsize);
      if (status == ERROR_SUCCESS)
	
	{
	  if (vtype == REG_SZ)
	    ...
	    {
	      if (debugflag)
		printf ("SIOD_LIB value wrong type %d\n", vtype);
	      siod_lib_arg[2] = 0;
	    }
	}
...
/* let the old style environment variable over-ride the
   ...
   for debugging. */ 
    if (((e = getenv ("SIOD_LIB")) && *e) || ((e = &siod_lib_arg[2]) && *e))
  ...
  if (!((pmain = (LPVOID) GetProcAddress (handle, "siod_main")) && 
	 (pshuffle_args =
	  (LPVOID) GetProcAddress (handle, "siod_shuffle_args"))
	 && (pcla = (LPVOID) GetProcAddress (handle, "process_cla"))))
    
    {
      fprintf (stderr,
	       "error code %d, cannot find siod calls in library %s\n",
	       GetLastError (), f);
      return (EXIT_FAILURE);
    }
  if (nargc > 0)
  ...
  (*pcla) ((sizeof (siod_argv) / sizeof (char *)) - 
	    ((siod_lib_arg[2]) ? 0 : 1), siod_argv, 1);
