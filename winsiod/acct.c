...
decode_utmp (struct utmp *p)
{
  return (symalist ("user", strcons (SAFE_STRLEN (p->ut_user), p->ut_user),
		    ...
		    (p->ut_type ==
		     LOGIN_PROCESS) ? cintern ("LOGIN_PROCESS") : (p->ut_type
								   ==
								   USER_PROCESS)
		    ? cintern ("USER_PROCESS") : (p->ut_type ==
						  DEAD_PROCESS) ?
		    cintern ("DEAD_PROCESS") :
#ifdef ACCOUNTING
		    (p->ut_type == ACCOUNTING) ? cintern ("ACCOUNTING") :
#endif
		    flocons (p->ut_type), "pid", flocons (p->ut_pid),
#if defined(__osf__)
...
LISP
decode_acct (struct acct * p)
{
  LISP flags = NIL;
  if (p->ac_flag & AFORK)
    flags = cons (cintern ("FORK"), flags);
  if (p->ac_flag & ASU)
    flags = cons (cintern ("SU"), flags);
  if (p->ac_flag & ACOMPAT)
    flags = cons (cintern ("COMPAT"), flags);
  if (p->ac_flag & ACORE)
    flags = cons (cintern ("CORE"), flags);
  if (p->ac_flag & AXSIG)
    flags = cons (cintern ("XSIG"), flags);
  if (p->ac_flag & ACCTF)
    flags = cons (flocons ((p->ac_flag & ACCTF) >> 6), flags);
  return (symalist ("comm", strcons (SAFE_STRLEN (p->ac_comm), p->ac_comm),
		    ...
		    "flag", flags, "stat", flocons (p->ac_stat), NULL));
}
...
decode_tacct (struct tacct *p)
{
  return (symalist ("uid", flocons (p->ta_uid),
		    ...
		    "pc", flocons (p->ta_pc),
		    "sc", flocons (p->ta_sc),
		    "dc", flocons (p->ta_dc), NULL));
}

LISP
ldecode_tacct (LISP l)
{
  char *buffer;
  long n;
  buffer = get_c_string_dim (l, &n);
...
init_acct (void)
{
  setvar (cintern ("UTMP_FILE"), strcons (strlen (UTMP_FILE), UTMP_FILE),
	  NIL);
  setvar (cintern ("WTMP_FILE"), strcons (strlen (WTMP_FILE), WTMP_FILE),
	  NIL);
