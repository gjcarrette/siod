...
static LISP
err_token_overflow (void)
{
  return (err ("token_overflow", NIL));
}

LISP
pratt_read_token (LISP buffer, LISP chars, LISP stream)
{
  LISP c, tail;
  char *tk;
  long j = 0, tkdim;
  ...
  while (1)
    if NULLP
      (c = lgetc (stream)) return (get_eof_val ());
    else if EQ
      (sym_nl_whitespace, tail = aref1 (chars, c))
	while (NNULLP (c = lgetc (stream)) && (get_c_long (c) != '\n'));
    else if NEQ
      (sym_whitespace, tail) break;
  if EQ
    (sym_string_delim, tail) while (NNULLP (c = lgetc (stream)))
      if EQ
	(sym_back_slash, aref1 (chars, c))
	{
	...
	(sym_string_delim, aref1 (chars, c)) return (strcons (j, tk));
      else if (j < tkdim)
	tk[j++] = (char) get_c_long (c);
      else
	err_token_overflow ();
  if EQ
    (tail, sym_back_slash) c = lgetc (stream);
  if (j < tkdim)
    tk[j++] = (char) get_c_long (c);
  else
    err_token_overflow ();
  if NULLP
    (tail) return (lreadtk (tk, j));
  else if CONSP
    (tail) while (1)
      if NULLP
	(c = lgetc (stream)) return (lreadtk (tk, j));
      else if NNULLP
	(tail = assq (c, tail)) if (j < tkdim)
	  tk[j++] = (char) get_c_long (c);
	else
	  err_token_overflow ();
      else
	{
	  if NEQ
	    (sym_whitespace, aref1 (chars, c)) lungetc (c, stream);
	  return (lreadtk (tk, j));
	}
  while (1)
    if NULLP
      ...
      (sym_back_slash, tail = aref1 (chars, c))
      {
	c = lgetc (stream);
	if (j < tkdim)
	  tk[j++] = (char) get_c_long (c);
      ...
      {
	if NEQ
	  (sym_whitespace, tail) lungetc (c, stream);
	return (lreadtk (tk, j));
      }
    else if (j < tkdim)
      tk[j++] = (char) get_c_long (c);
