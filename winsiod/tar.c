...
static long
safe_atol8 (const char *ptr, size_t len)
{
  long x = 0;
  int c;
  size_t j;
...
static struct tar_header *
get_tar_header (LISP bytes)
{
  long n;
  struct tar_header *h;
...
LISP
checksum_tar_header (LISP bytes, LISP whole)
{
  struct tar_header *h = get_tar_header (bytes);
  if NNULLP
    (whole)
...
init_tar (void)
{
  ...
  setvar (cintern ("*tar-header-size*"),
	  flocons (sizeof (struct tar_header)), NIL);
  init_tar_version ();
}
