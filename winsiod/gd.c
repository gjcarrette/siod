...
LISP
lgdImageCreate (LISP sx, LISP sy)
{
  LISP result;
  long iflag;
  result = cons (NIL, NIL);
...
gdPointPtr
get_gdPointPtr (LISP ptr, long *n)
{
  gdPointPtr pt;
...
LISP
lgdPointx (LISP ptr, LISP j, LISP value)
{
  long n, i;
  gdPointPtr pt;
  ...
  i = get_c_long (j);
  if ((i < 0) || (i >= n))
    err ("index out of range", j);
  if NNULLP
    (value) pt[i].x = get_c_long (value);
  else
...
LISP
lgdPointy (LISP ptr, LISP j, LISP value)
{
  long n, i;
  gdPointPtr pt;
  pt = get_gdPointPtr (ptr, &n);
...
#if 1
/* Note: If you hack your gd library source like I did you can add a function that
   produces a gif string directly in memory without the need for a temporary file.
   Without that however, Bill Hails <bill@clic.co.uk> recommends this: */

#include <sys/stat.h>
#include <unistd.h>

void
gdImageGifmem (gdImagePtr im, char *buffer, size_t * len)
{
  FILE *f;
  char tmpname[32];
  struct stat sb;
...
LISP
lgdImagePolygon (LISP im, LISP points, LISP color)
{
  gdPointPtr pt;
  long n;
  pt = get_gdPointPtr (points, &n);
  gdImagePolygon (get_gdImagePtr (im), pt, n, get_c_long (color));
  return (NIL);
}

LISP
lgdImageFilledPolygon (LISP im, LISP points, LISP color)
{
  gdPointPtr pt;
  long n;
  pt = get_gdPointPtr (points, &n);
  gdImageFilledPolygon (get_gdImagePtr (im), pt, n, get_c_long (color));
  return (NIL);
}

LISP
lgdImageRectangle (LISP l)
{
  gdImagePtr im;
  int x1, y1, x2, y2, color;
  im = get_gdImagePtr (poparg (&l, NIL));
...
LISP
lgdImageFilledRectangle (LISP l)
{
  gdImagePtr im;
  int x1, y1, x2, y2, color;
  im = get_gdImagePtr (poparg (&l, NIL));
...
LISP
lgdImageColorTransparent (LISP im, LISP color)
{
...
LISP
lgdImageInterlace (LISP im, LISP interlace)
{
...
void
gdimage_prin1 (LISP ptr, struct gen_printio *f)
{
  char buff[256];
  gdImagePtr im;
  if ((im = (gdImagePtr) ptr->storage_as.string.data))
    sprintf (buff, "#<GDIMAGE %p %d by %d>", im, im->sx, im->sy);
  else
...
void
gdfont_prin1 (LISP ptr, struct gen_printio *f)
{
  char buff[256];
  gdFontPtr fnt;
  if ((fnt = (gdFontPtr) ptr->storage_as.string.data))
    sprintf (buff, "#<GDFONT %p %d by %d>", fnt, fnt->w, fnt->h);
  else
...
void
gdpoint_prin1 (LISP ptr, struct gen_printio *f)
{
  char buff[256];
  gdPointPtr pt;
  pt = (gdPointPtr) ptr->storage_as.string.data;
  sprintf (buff, "#<GDPOINT %p %ld>", pt, ptr->storage_as.string.dim);
  gput_st (f, buff);
}

void
gdpoint_gc_free (LISP ptr)
{
  gdPointPtr pt;
  if ((pt = (gdPointPtr) ptr->storage_as.string.data))
    {
...
init_gd (void)
{
  ...
  set_gc_hooks (tc_gdimage, NULL, NULL, NULL, gdimage_gc_free, &j);
  set_print_hooks (tc_gdimage, gdimage_prin1);
  tc_gdfont = allocate_user_tc ();
  set_gc_hooks (tc_gdfont, NULL, NULL, NULL, gdfont_gc_free, &j);
  set_print_hooks (tc_gdfont, gdfont_prin1);
  tc_gdpoint = allocate_user_tc ();
  set_gc_hooks (tc_gdpoint, NULL, NULL, NULL, gdpoint_gc_free, &j);
  set_print_hooks (tc_gdpoint, gdpoint_prin1);
  setvar (cintern ("gdFontGiant"), lcgdFontCreate (gdFontGiant), NIL);
  setvar (cintern ("gdFontLarge"), lcgdFontCreate (gdFontLarge), NIL);
  setvar (cintern ("gdFontMediumBold"),
	  lcgdFontCreate (gdFontMediumBold), NIL);
  setvar (cintern ("gdFontSmall"), lcgdFontCreate (gdFontSmall), NIL);
  setvar (cintern ("gdFontTiny"), lcgdFontCreate (gdFontTiny), NIL);
